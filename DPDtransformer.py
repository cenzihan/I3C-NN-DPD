...
config=dict(
  ...
  activateF1="relu",
  activateF2="nn.Tanh()",
  # DatasetName = '_GMP_M5_L200k'
  PAname = '5_M5_L200k_dpd'
)
...
M = wandb.config.M
# 定义网络结构
class DPDtransformer(nn.Module):
    def __init__(self):
        super(DPDtransformer, self).__init__()
        # embed_size = 16
        # num_layers = 3
        ...
        # num_layers = 1
        feature_num = 10
        encoder_layer = nn.TransformerEncoderLayer(d_model=5, nhead=1, dim_feedforward=5, activation='relu', batch_first=True, norm_first=True)
...
if __name__ == "__main__":
    ...
    torch.save(net.state_dict(), './coefs/DPDtransformer.pth')
